(function(id, t) {
	...
	var doc = document,
		...
		addHtml = function() {
			...
			oBtn.innerHTML = '<span></span><i class="lb"></i><i class="rb"></i>';
			oId.appendChild(oBg);
		...
		init = function() {
			...
			var turnPic = getClass(oId, 'ul', 'turn-pic')[0];
			var oLi = turnPic.getElementsByTagName('li');
			var oLb = getClass(oId, 'i', 'lb')[0];
			var oRb = getClass(oId, 'i', 'rb')[0];
			oLb.onmouseover = function() {
				this.style.backgroundPosition = '-75px 0';
			}
			...
			oRb.onmouseover = function() {
				this.style.backgroundPosition = '-112px 0';
			}
			oRb.onmouseout = function() {
				this.style.backgroundPosition = '-37.5px 0';
			}
