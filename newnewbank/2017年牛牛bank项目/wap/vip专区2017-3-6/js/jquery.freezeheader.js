...
(function($) {
	...
	$.fn.freezeHeader = function(params) {
		...
		function freezeHeader(elem) {
			...
			if(elem.length > 0 && elem.hasClass("select-no")) {
				var obj = {
					id: idObj,
					grid: elem,
					container: null,
				...
				obj.header = obj.grid.find('.suspended');
				if(params && params.height !== undefined) {
					...
					if(obj.header.offset() != null) {
						if(limiteAlcancado(obj, params)) {
							...
							if(($(document).scrollTop() > obj.header.offset().top)) {
								...
								obj.container.css("top", (obj.grid.find(".single-data:last").offset().top - obj.header.height()) + "px");
							} else {
		...
		function cloneHeaderRow(obj) {
			obj.container.html('');
			obj.container.val('');
			var tabela = $('<div class="select-no" style="margin: 0 0;"></div>');
			...
			tabela.append('<div class="suspended">' + obj.header.html() + '</div>');
			...
			if(params && params.height !== undefined) {
				...
				obj.container.css("top", obj.scroller.find(".suspended > .single-data").innerHeight() + "px");
				obj.container.css("position", "absolute");
